...
material_properties& material_properties::add(G4String const& key, G4double value, G4bool boolean) {
  table->AddConstProperty(key, value, boolean);
  return *this; //ITS ASO
}

material_properties& material_properties::add(G4String const& key, vec const& energies, vec const& values, const G4int integer) {
  table -> AddProperty(key, energies, values, integer); 
  return *this; //ITS ASO
}
